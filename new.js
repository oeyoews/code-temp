...
(function () {
  ...
  TWSRWidget.prototype.GetScheduledCards = function (title) {
    ...
    if (tiddlers.length == 0) {
      ...
      filter =
        tagFilter +
        ':filter[get[twsr_interval]compare:date:lt[' +
  ...
  TWSRWidget.prototype.UpdateCardSM2 = function (title, grade) {
    ...
    $tw.wiki.setText(
      ...
      'twsr_interval',
      undefined,
      $tw.utils.stringifyDate(reviewDate)
    );
  ...
  TWSRWidget.prototype.ShowCards = function (parent, nextSibling) {
    ...
    var gradeDiv = _this.CreateElement('div', '', {
      float: 'left',
      display: 'none',
    });
    ...
    var showAnswer = _this.CreateElement('button', '', {
      float: 'left',
      display: 'none',
    });
    ...
    if (!settingsDiv.isTiddlyWikiFakeDom) {
      settingsDiv.setAttribute(
        'title',
        _this.twsr_scheduled_tip + String(tiddlers.length)
      );
    }
    ...
    var ShowCard = function (tiddler) {
      ...
      var t = $tw.wiki.makeTranscludeWidget(tiddler, {
        document: document,
        variables: { currentTiddler: tiddler },
      });
    ...
    var OnClickGrade = function (grade) {
      ...
      if (!settingsDiv.isTiddlyWikiFakeDom) {
        settingsDiv.setAttribute(
          'title',
          _this.twsr_scheduled_tip + String(tiddlers.length - index)
        );
      }
      if (index >= limit) {
  ...
  TWSRWidget.prototype.GetConfigTiddlers = function () {
    ...
    for (i = 0; i < tiddlers.length; i++) {
      ...
      var scheduled_tip =
        tiddler.getFieldString('twsr_cfg_cards_scheduled') + ' ';
      var finished = tiddler.getFieldString('twsr_cfg_cards_finished') + ' ';
      var nothing_scheduled =
        tiddler.getFieldString('twsr_cfg_nothing_scheduled') + ' ';
        ...
        if (names.length == numbers.length) {
          ...
          console.error(
            'tswr config tiddler : ' +
              ...
              ' malformed, grade names do not match numbers'
          );
        }
      } else {
        console.error(
  ...
  TWSRWidget.prototype.InitTWSR = function () {
    ...
    this.m_tiddler_name = this.getAttribute(
      g_src,
      this.getVariable('currentTiddler')
    );
    this.m_tags = $tw.utils.parseStringArray(this.getAttribute('tags', '')); //we should be passed tags
    this.m_cfgTags = $tw.utils.parseStringArray(
      this.getAttribute('cfg', '$:/tags/twsr')
    ); //match the config we want, it not passed with use the twsr default
  ...
  TWSRWidget.prototype.refresh = function (changedTiddlers) {
    ...
    if (
      ...
      changedAttributes.m_tiddler_name
    ) {
      this.refreshSelf();
      return true;
    }
  ...
  TWSRAnswer.prototype.render = function (parent, nextSibling) {
    ...
    if (!g_twsrActive) {
      ...
      ///
      if (
        tags.indexOf('$:/tags/twsr/hideAnswer') >= 0 ||
        tags.indexOf('$:/tags/twsr/hideAnswers') >= 0
      ) {
        locallyActive = true;
      }
    }
